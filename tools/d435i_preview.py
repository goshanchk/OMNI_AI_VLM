    ...
    *,
    ...
    while True:
        ...
        try:
            ...
            ok_color, color_encoded = cv2.imencode('.jpg', request.color_bgr, encode_params)
            if not ok_color:
                state.error_text = 'Failed to encode RGB frame'
                continue

            files = {'image': ('frame.jpg', color_encoded.tobytes(), 'image/jpeg')}
            if send_depth and request.depth_image is not None:
                ok_depth, depth_encoded = cv2.imencode('.png', request.depth_image)
                if not ok_depth:
                    state.error_text = 'Failed to encode depth frame'
                    continue
                files['depth'] = ('depth.png', depth_encoded.tobytes(), 'image/png')

            data = {'intrinsics': json.dumps(request.intrinsics), 'depth_scale': str(depth_scale)}
            if preferred_label:
...
def main() -> None:
    ...
    parser.add_argument('--infer-every', type=int, default=15, help='Prepare a request every N frames')
    ...
    parser.add_argument(
        '--task-prompt',
        default=None,
        help='Free-form task prompt that states the fly-to object and projection target, e.g. "find person and project on wall"',
    )
    parser.add_argument('--send-depth', action='store_true', help='Upload aligned depth PNG to the server')
    parser.add_argument('--no-display', action='store_true', help='Disable local OpenCV preview to reduce CPU load on Orange Pi')
